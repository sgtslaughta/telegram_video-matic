...
async def reconcile(request: Request, bg: BackgroundTasks, dry_run: bool = False):
...
@router.post("/rematch")
async def rematch(request: Request, bg: BackgroundTasks, dry_run: bool = False,
                  rescore: bool = False):
...
@router.post("/import")
async def import_library(request: Request, bg: BackgroundTasks,
...
async def report(name: str, request: Request):
    """Last result of import / rematch / reconcile (404 until one has run)."""
    rep = _service(request).reports.get(name)
    if rep is None:
