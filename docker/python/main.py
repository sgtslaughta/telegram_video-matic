...
def config():
    ...
    if 'api_id' not in st.session_state:
        if 'API_ID' in os.environ:
            st.session_state.api_id = os.environ['API_ID']
        else:
            st.session_state.api_id = None
    if 'api_hash' not in st.session_state:
        if 'API_HASH' in os.environ:
            st.session_state.api_hash = os.environ['API_HASH']
        else:
            st.session_state.api_hash = None
    if 'phone' not in st.session_state:
        if 'PHONE' in os.environ:
            st.session_state.phone = os.environ['PHONE']
        else:
            st.session_state.phone = None
    if 'sportsdb_api' not in st.session_state:
        if 'SDB_API' in os.environ:
            st.session_state.sportsdb_api = os.environ['SDB_API']
        else:
            st.session_state.sportsdb_api = None
    if 'root_dl_path' not in st.session_state:
        if 'ROOT_DL_PATH' in os.environ:
            st.session_state.root_dl_path = os.environ['ROOT_DL_PATH']
        else:
